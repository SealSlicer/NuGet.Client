...
namespace NuGet.Commands.Test
{
    public class MSBuildRestoreUtilityTests
    {
        ...
        [InlineData("           false    ", false)]
        public void MSBuildRestoreUtility_GetPackageSpec_CPVM_VersionOverrideCanBeDisabled(string isCentralPackageVersionOverrideEnabled, bool expected)
        {
            var projectName = "alegacycpvm";
            using (var workingDir = TestDirectory.Create())
            {
                ...
                items.Add(new Dictionary<string, string>()
                {
                    ...
                    { "ProjectPath", project1Path },
                    { "_CentralPackageVersionsEnabled", "true"},
                    { ProjectBuildProperties.EnablePackageVersionOverride, isCentralPackageVersionOverrideEnabled }
                });
                ...
                if (expected)
                {
                    Assert.True(project1Spec.RestoreMetadata.CentralPackageVersionOverrideEnabled);
                }
                else
                {
                    Assert.False(project1Spec.RestoreMetadata.CentralPackageVersionOverrideEnabled);
                }
            }
        }
